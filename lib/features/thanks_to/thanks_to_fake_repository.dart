import 'thanks_to_dto.dart';

class ThanksToFakeRepository {
  List<ThanksToDto> getAll() {
    return [
      ThanksToDto(
        'GDG Formosa',
        'Por brindarme el espacio y la buena onda :)',
        'https://pbs.twimg.com/profile_images/1117957521459875840/8Hc4y-FM_400x400.png',
        'https://www.meetup.com/es-ES/GDG-Formosa/',
      ),
      ThanksToDto(
        'Clarika',
        'Por el apoyo y el sponsoreo :) ',
        'https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTks9uLHScuwYBlkAXaxUr9yI7eBA-2MosoNmIpLh_ham0dJEZ7&usqp=CAU',
        'https://clarikagroup.com/',
      ),
      ThanksToDto(
        'Serenader2014',
        'Flutter Package: carousel_slider',
        'https://avatars2.githubusercontent.com/u/6716522?s=460&v=4',
        'https://github.com/serenader2014/flutter_carousel_slider',
      ),
      ThanksToDto(
        'Aagarwal1012',
        'Flutter Package: intro_views_flutter',
        'https://avatars3.githubusercontent.com/u/34043023?s=460&u=944bb4530a3f918cedb0bd7634b035fa4e42b522&v=4',
        'https://github.com/aagarwal1012/IntroViews-Flutter',
      ),
      ThanksToDto(
        'pedromassango',
        'Flutter Package: bottom_navy_bar',
        'https://avatars2.githubusercontent.com/u/33294549?s=460&u=c832597c58517e4eb4ad1df0c5c98b0998a973af&v=4',
        'https://github.com/pedromassango/bottom_navy_bar',
      ),
      ThanksToDto(
        'hnvn',
        'Flutter Package: shimmer',
        'https://avatars1.githubusercontent.com/u/5468513?s=460&u=59d0ede1666a723eecdcb51a794466ba98e1f408&v=4',
        'https://github.com/hnvn/flutter_shimmer',
      ),
      ThanksToDto(
        'ShaunRain',
        'Flutter Package: flutter_tindercard',
        'https://avatars1.githubusercontent.com/u/7543270?s=460&u=0b090aefaeda5da401062785db265e4f59f00216&v=4',
        'https://github.com/ShaunRain/flutter_tindercard',
      ),
    ];
  }
}
